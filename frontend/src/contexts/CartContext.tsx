...
export function CartProvider({ children }: { children: React.ReactNode }) {
  const [cart, setCart] = useState<CartItem[]>(() => {
    try {
      const saved = readLocal("sofa_co_cart");
  ...
  const [sessionKey] = useState(() => {
    try {
      const existing = readLocal("sofa_co_cart_session");
      if (existing) return existing;
      const created = `cart_${crypto.randomUUID()}`;
      writeLocal("sofa_co_cart_session", created);
      return created;
    } catch {
  ...
  useEffect(() => {
    try {
      writeLocal("sofa_co_cart", JSON.stringify(cart));
    } catch {}
  }, [cart]);
