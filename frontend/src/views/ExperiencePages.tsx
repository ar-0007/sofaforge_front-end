...
export function Wishlist() {
  ...
  useEffect(() => {
    const stored = readLocal("sofa-co-wishlist");
    if (!stored) return;
    try {
      setSaved(JSON.parse(stored) as number[]);
    } catch {
      // corrupted wishlist payload — start clean
    }
  }, []);
  ...
  const toggleSaved = (id: number) => {
    ...
    setSaved(next);
    writeLocal("sofa-co-wishlist", JSON.stringify(next));
  };
